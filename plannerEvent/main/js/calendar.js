...
const eventSection = document.getElementById('event__modal-section');
...
const updateCalendar = () => {
  ...
  fetch('https://planner.rdclr.ru/api/events?populate=*')
        ...
        for (let j = 0; j < 7; j++) {
          ...
          if (i === 0 && j < firstDayOfWeek - 1) {
            ...
            for (let count = 0; count < eventInfo.length; count++) {
              let dateCell = formatDateCalendar(new Date(year, month, day));
              if (new Date(eventInfo[count].dateStart).toISOString().split('T')[0] === dateCell) {
                let eventTitles = '';
                for (let count = 0; count < eventInfo.length; count++) {
                  let dateCell = formatDateCalendar(new Date(year, month, day));
                  if (formatDateCalendar(new Date(eventInfo[count].dateStart).toISOString().split('T')[0]) === dateCell) {
          ...
          row.innerHTML += `<td class="event-date" data-date="${year}-${month + 1}-${day}">
                              ${cellContent}
                            </td>`;
        }
...
// Обработчик для отображения информации о событии при клике на title-event-text
calendar.addEventListener('click', (event) => {
  ...
  if (target.classList.contains('title-event-text')) {
    ...
    if (eventInfoData) {
      ...
      const membersPeople = document.getElementById('membersPeopleContainer')
      membersPeople.innerHTML = ''; // Очищаем контейнер
      ...
      organizerImg.src = 'img/avatar.png'; // Установите путь к изображению организатора, если необходимо
      ...
      // Создаем и добавляем блоки для первых 5 участников
    ...
    // Очистите содержимое контейнера перед добавлением новых фотографий
    galleryPhotoContainer.innerHTML = '';

    // Получите массив фотографий из eventInfo (предположим, что eventInfo это массив объектов)
    const index = eventInfo.findIndex(obj => obj.id == eventId);
    const photos = eventInfo[index].photos; // Предполагаю, что фотографии хранятся в первом объекте eventInfo
    
    // Создайте цикл для добавления фотографий
    if ((photos) && (photos.length !== 0)) {
      ...
      const photosEvent = document.querySelectorAll('.photo');
      ...
      // Функция для отображения четырех фотографий, включая частично видимую
      function showPhotos() {
        hideAllPhotos();
        for (let i = 0; i < 3; i++) {
      ...
      // Первичное отображение фотографий
      showPhotos();
    }
